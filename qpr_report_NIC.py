...
# URL for report page
URL = 'http://wbscps.in/Home_MIS/Load/jjb_rprt_I_page'
# report quater
...
while True:
    ...
    for row in rows[1:]:
        ...
        # check for quater
        if table_cols[1].string.strip() != QUATER:
            continue
        print("-----Found data for " + table_cols[2].string)
        links = row.find_all("a", string = 'View')
        link = str(links[0].get('onclick')).split("'")[1]
        cols = [table_cols[2].string, table_cols[1].string.strip(), link]
        values.append(cols)
    page_no = page_no + 20
